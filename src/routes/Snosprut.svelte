<script lang="ts">
  ...
  const createSnowParticles = () => {
    return Array.from({ length: snowCount }, () => ({
      position: [
        Math.random() * 0.1, // X: tilfeldig mellom -10 og 10
        Math.random() * 0.1, // Y: tilfeldig mellom -10 og 10
...
<Theatre config={{ state: state }} studio={{ hide: true }}>
  <Sequence autoplay>
    <SheetObject key="Snosprut" props={{ scale: 0, opacity: 0, rotationx: 0, rotationy: 0, rotationz: 0, positionx: 0, positiony: 0, positionz: 0, }} let:values>
