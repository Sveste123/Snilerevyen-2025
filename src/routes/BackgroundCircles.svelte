  ...
  const createSnowParticles = () => {
    return Array.from({ length: snowCount }, () => ({
      position: [
        ...
        Math.random() * 4.1 - 3.9, // Y: tilfeldig mellom -10 og 10
        Math.random() * 0.1  // Z: tilfeldig mellom -10 og 10
      ],
      scale: Math.random() * 0.1 // Tilfeldig mellom 0.005 og 0.04
    }));
  };
...
<Theatre config={{ state: state }} studio={{ hide: true }}>
  <Sequence autoplay>
    <SheetObject key="DLB" props={{ scale: 0, opacity: 0, rotationx: 0, rotationy: 0, rotationz: 0, positionx: 0, positiony: 0, positionz: 0, }} let:values>
